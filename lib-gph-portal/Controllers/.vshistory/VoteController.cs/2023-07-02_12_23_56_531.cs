...
namespace sa.gov.libgph.Controllers
{
    public class VoteController : SurfaceController
    {
        ...
        public void SubmitVote(string vote )
        {
            ...
            if (vote != null)
            {
                ...
                if (cookie == null)
                {
                    ...
                    RecordVote(vote, Request.Url.Host);
        ...
        public static void StoreInCookie(string cookieName, string cookieDomain, string keyName, string value, DateTime? expirationDate, bool httpOnly = true)
        {
        ...
        public static void RecordVote(string vote,string host)
        {
                ...
                using (SqlCommand cmd = new SqlCommand("IF NOT EXISTS (SELECT * FROM visitorData WHERE IPAddress = @ipAddress) INSERT INTO visitorData (IPAddress, SessionID) VALUES (@ipAddress, @sessionId)", conn))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        using (SqlCommand cmd2 = new SqlCommand("record_vote", conn))
                        {
                            ...
                            conn.Close();
                            StoreInCookie("lib_Vote", host, "vote", vote, DateTime.Now.AddDays(15));
