    ...
    public class VoteController : SurfaceController
    {
        ...
        public static void RecordVote(string vote)
        {
            using (SqlConnection conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["umbracoDbDSN"].ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("IF NOT EXISTS (SELECT * FROM visitorData WHERE IPAddress = @ipAddress) INSERT INTO visitorData (IPAddress, SessionID, SomeOtherField) VALUES (@ipAddress, @sessionId, @someOtherValue)", conn))
                {
